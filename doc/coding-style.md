# Bolt Coding Style and Best Practices

The goal of this document is to make Bolt developers more productive by
...
* For multi-line comments:
  * Bolt will follow the doxygen comment style. 
...
* For assertions and other types of validation, use `BOLT_CHECK_*` macros
  * `BOLT_CHECK_*` will categorize the error to be an internal runtime error.
  * `BOLT_USER_CHECK_*` will categorize the error to be a user error.
* Use `BOLT_FAIL()` or `BOLT_USER_FAIL()` to inadvertently throw an
  exception:
  * `BOLT_FAIL("Illegal state");`
* Use `BOLT_UNREACHABLE()` when a particular branch/block should never be
  executed, such as in a switch statement with an invalid `default:` block.
* Use `BOLT_NYI()` for features or code paths that are not implemented yet.
* When comparing two values/expressions, prefer to use:
  * `BOLT_CHECK_LT(idx, children_.size());`
* Rather than:
  * `BOLT_CHECK(idx < children_.size());`
...
* All macro also provide the following optional features:
  * Specifying error code (error code listed in
    `bolt/common/base/BoltException.h`): 
    * `BOLT_CHECK_EQ(v1[, v2[, error_code]]);`
    * `BOLT_USER_CHECK_EQ(v1[, v2[, error_code]])`
  * Appending error message:
    * `BOLT_CHECK_EQ(v1, v2, “Some error message”)`
    * `BOLT_USER_CHECK_EQ(v1, v2, “Some error message”)`
  * Error message formatting via fmt:
    * `BOLT_USER_CHECK_EQ(v1, v2, “My complex error message {} and {}”, str,
      i)`
...
using ContinuePromise = BoltPromise<bool>;
```
